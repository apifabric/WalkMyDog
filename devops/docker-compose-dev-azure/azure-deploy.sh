...
resourcename="walkmydog"  # lower case, only
resourcegroup="walkmydog_rg"
...
# git clone https://github.com/apilogicserver/walkmydog.git
# cd walkmydog
...
az group create --name walkmydog_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group walkmydog_rg --sku S1 --is-linux
az container create --resource-group walkmydog_rg --name walkmydog --image apilogicserver/walkmydog:latest --dns-name-label walkmydog --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//walkmydog.westus.azurecontainer.io:5656'
